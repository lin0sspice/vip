...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            // Note: CvipcoinSecret::SetString tests isValid, whereas CvipcoinAddress does not!
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        else
        {
            ...
            CvipcoinAddress addrOut;
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CvipcoinAddressVisitor(&addrOut), dest), "encode dest: " + strTest);
            BOOST_CHECK_MESSAGE(addrOut.ToString() == exp_base58string, "mismatch: " + strTest);
        }
    }

    // Visiting a CNoDestination must fail
    CvipcoinAddress dummyAddr;
    CTxDestination nodest = CNoDestination();
    BOOST_CHECK(!boost::apply_visitor(CvipcoinAddressVisitor(&dummyAddr), nodest));
