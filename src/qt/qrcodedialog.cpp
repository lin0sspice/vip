...
{
    QString ret = QString("vipcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VIP unit input in lnReqAmount, we generate the URI with VIP as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(vipcoinUnits::format(vipcoinUnits::VIP, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
